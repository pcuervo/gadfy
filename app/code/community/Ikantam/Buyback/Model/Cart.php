...
class Ikantam_Buyback_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    /**
    ...
    {
        return Mage::getResourceSingleton('buyback/cart');
    }
    ...
    public function getQuote()
    {
        if (!$this->hasData('quote')) {
            $this->setData('quote', $this->getBuybackSession()->getQuote());
        }
    ...
    public function init()
    {
         ...
         */
        if ($this->getBuybackSession()->getCheckoutState() !== Mage_Checkout_Model_Session::CHECKOUT_STATE_BEGIN) {
            $this->getQuote()
                ->removeAllAddresses()
                ->removePayment();
            $this->getBuybackSession()->resetCheckout();
        }
    ...
    public function addProduct($productInfo, $requestInfo=null)
    {
        ...
        if ($productId) {
            try {
                ...
                $this->getBuybackSession()->setUseNotice(false);
                $result = $e->getMessage();
            }
            /**
            ...
            if (is_string($result)) {
                ...
                $this->getBuybackSession()->setRedirectUrl($redirectUrl);
                if ($this->getBuybackSession()->getUseNotice() === null) {
                    $this->getBuybackSession()->setUseNotice(true);
                }
        ...
        $this->getBuybackSession()->setLastAddedProductId($productId);
        return $this;
    }
    ...
    public function addProductsByIds($productIds)
    {
        ...
        if (!empty($productIds)) {
            ...
            if (!$allAvailable) {
                $this->getBuybackSession()->addError(
    ...
    {
        Mage::dispatchEvent('buyback_cart_update_items_before', array('cart'=>$this, 'info'=>$data));

        /* @var $messageFactory Mage_Core_Model_Message */
        $messageFactory = Mage::getSingleton('core/message');
        $session = $this->getBuybackSession();
    ...
    {
        Mage::dispatchEvent('buyback_cart_save_before', array('cart'=>$this));

        $this->getQuote()->getBillingAddress();
        $this->getQuote()->getShippingAddress()->setCollectShippingRates(true);
        $this->getQuote()->collectTotals();
        $this->getQuote()->save();
        $this->getBuybackSession()->setQuoteId($this->getQuote()->getId());
        /**
         * Cart save usually called after changes with cart items.
         */
        Mage::dispatchEvent('buyback_cart_save_after', array('cart'=>$this));
        return $this;
    }
    ...
    {
        $quoteId = Mage::getSingleton('buyback/session')->getQuoteId();
        ...
        if (!$quoteId && Mage::getSingleton('customer/session')->isLoggedIn()) {
            $quote = Mage::getSingleton('buyback/session')->getQuote();
            $quoteId = Mage::getSingleton('buyback/session')->getQuoteId();
        }
    ...
    public function updateItem($itemId, $requestInfo = null, $updatingParams = null)
    {
        ...
        Mage::dispatchEvent('buyback_cart_product_update_after', array(
            'quote_item' => $result,
            'product' => $product
        ));
        $this->getBuybackSession()->setLastAddedProductId($productId);
