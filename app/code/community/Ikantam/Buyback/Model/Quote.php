...
class Ikantam_Buyback_Model_Quote extends Mage_Sales_Model_Quote
{
    protected $_eventPrefix = 'buyback_quote';
...
//            $this->setShippingAddress($shippingAddress);
    ...
    public function getAddressesCollection()
    {
        if (is_null($this->_addresses)) {
            $this->_addresses = Mage::getModel('buyback/quote_address')->getCollection()
...
//
    ...
    protected function _getAddressByType($type)
    {
        ...
        $address = Mage::getModel('buyback/quote_address')->setAddressType($type);
...
//
    ...
    public function removeItem($itemId)
    {
        ...
        if ($item) {
            ...
            Mage::dispatchEvent('buyback_quote_remove_item', array('quote_item' => $item));
        }
    ...
    public function addItem(Mage_Sales_Model_Quote_Item $item)
    {
        ...
        if ($item->isNominal() && $this->hasItems() || $this->hasNominalItems()) {
            Mage::throwException(
                Mage::helper('buyback')->__('Nominal item can be purchased standalone only. To proceed please remove other items from the quote.')
            );
        }
        ...
        if (!$item->getId()) {
            ...
            Mage::dispatchEvent('buyback_quote_add_item', array('quote_item' => $item));
        }
        return $this;
    }
     ...
     * @return Ikantam_Buyback_Model_Quote_Item|string
     */
    public function addProductAdvanced(Mage_Catalog_Model_Product $product, $request = null, $processMode = null)
    {
        ...
        if (!($request instanceof Varien_Object)) {
            Mage::throwException(Mage::helper('buyback')->__('Invalid request for adding product to quote.'));
        }
        ...
        Mage::dispatchEvent('buyback_quote_product_add_after', array('items' => $items));
    ...
    protected function _addCatalogProduct(Mage_Catalog_Model_Product $product, $qty = 1)
    {
        ...
        if (!$item) {
            $item = Mage::getModel('buyback/quote_item');
            $item->setQuote($this);
    ...
    public function updateItem($itemId, $buyRequest, $params = null)
    {
        $item = $this->getItemById($itemId);
        if (!$item) {
            Mage::throwException(Mage::helper('buyback')->__('Wrong quote item id to update configuration.'));
        }
     ...
     *
     * @param   Mage_Catalog_Model_Product $product
     * @return  Ikantam_Buyback_Model_Quote_Item || false
     */
...
//            );
        ...
        Mage::helper('buyback')->checkQuoteAmount($this, $this->getGrandTotal());
        Mage::helper('buyback')->checkQuoteAmount($this, $this->getBaseGrandTotal());
...
//         Going through all quote addresses and merge their totals
    ...
    public function addErrorInfo($type = 'error', $origin = null, $code = null, $message = null, $additionalData = null)
    {
        if (!isset($this->_errorInfoGroups[$type])) {
            $this->_errorInfoGroups[$type] = Mage::getModel('buyback/status_list');
        }
    ...
    public function isVirtual()
    {
        ...
        foreach ($this->getItemsCollection() as $_item) {
            /* @var $_item Ikantam_Buyback_Model_Quote_Item */
