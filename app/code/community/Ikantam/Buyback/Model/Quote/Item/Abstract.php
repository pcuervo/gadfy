 ...
 */
abstract class Ikantam_Buyback_Model_Quote_Item_Abstract extends Mage_Core_Model_Abstract
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
    ...
    public function checkData()
    {
        ...
        try {
            ...
            $this->setHasError(true);
            $this->setMessage(Mage::helper('buyback')->__('Item qty declaration error.'));
        }
        ...
        try {
            ...
            $this->getQuote()->addMessage(
                Mage::helper('buyback')->__('Some of the products below do not have all the required options. Please edit them and configure all the required options.')
            );
        } catch (Exception $e) {
            $this->setHasError(true);
            $this->setMessage(Mage::helper('buyback')->__('Item options declaration error.'));
            $this->getQuote()->setHasError(true);
            $this->getQuote()->addMessage(Mage::helper('buyback')->__('Items options declaration error.'));
        }
