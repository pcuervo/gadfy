...
class Ikantam_Buyback_Model_Quote_Address extends Mage_Sales_Model_Quote_Address
{
    const RATES_FETCH = 1;
     ...
     * @var string
     */
    protected $_eventPrefix = 'buyback_quote_address';
    ...
    {
        $this->_init('buyback/quote_address');
    }
    ...
    {
        $this->_oldFieldsMap = Mage::helper('buyback')->getOldFieldMap('quote_address');
        return $this;
    }
    ...
    public function exportCustomerAddress()
    {
        ...
        Mage::helper('core')->copyFieldset('buyback_convert_quote_address', 'to_customer_address', $this, $address);
        return $address;
    }
    ...
    public function importOrderAddress(Mage_Sales_Model_Order_Address $address)
    {
        ...
        Mage::helper('core')->copyFieldset('buyback_convert_order_address', 'to_quote_address', $address, $this);
    ...
    public function getItemsCollection()
    {
        if (is_null($this->_items)) {
            $this->_items = Mage::getModel('buyback/quote_address_item')->getCollection()
    ...
    public function addItem(Mage_Sales_Model_Quote_Item_Abstract $item, $qty=null)
    {
        if ($item instanceof Mage_Sales_Model_Quote_Item) {
            ...
            $addressItem = Mage::getModel('buyback/quote_address_item')
                ->setAddress($this)
            ...
            if ($item->getHasChildren()) {
                foreach ($item->getChildren() as $child) {
                    $addressChildItem = Mage::getModel('buyback/quote_address_item')
                        ->setAddress($this)
    ...
    public function getShippingRatesCollection()
    {
        if (is_null($this->_rates)) {
            $this->_rates = Mage::getModel('buyback/quote_address_rate')->getCollection()
                ->setAddressFilter($this->getId());
    ...
    public function requestShippingRates(Mage_Sales_Model_Quote_Item_Abstract $item = null)
    {
        ...
        if ($result) {
            ...
            foreach ($shippingRates as $shippingRate) {
                $rate = Mage::getModel('buyback/quote_address_rate')
    ...
    public function getTotalCollector()
    {
        if ($this->_totalCollector === null) {
            $this->_totalCollector = Mage::getSingleton(
                'buyback/quote_address_total_collector',
    ...
    public function collectTotals()
    {
        ...
        foreach ($this->getTotalCollector()->getCollectors() as $model) {
            if ( get_class($model) == 'Ikantam_Buyback_Model_Quote_Address_Total_Grand' ||
                get_class($model) == 'Ikantam_Buyback_Model_Quote_Address_Total_Subtotal'
            ) {
                $model->collect($this);
            }
        }
    ...
    {
        if (is_array($total)) {
            $totalInstance = Mage::getModel('buyback/quote_address_total')
                ->setData($total);
    ...
    public function validateMinimumAmount()
    {
        ...
        if (!Mage::getStoreConfigFlag('buyback/minimum_order/active', $storeId)) {
            return true;
        }
        ...
        $amount = Mage::getStoreConfig('buyback/minimum_order/amount', $storeId);
