...
class Ikantam_Buyback_Model_Type_Onepage
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->_customerEmailExistsMessage = Mage::helper('buyback')->__('There is already a customer registered using this email address. Please login using this email address or enter a different email address to register your account.');
        $this->_checkoutSession = Mage::getSingleton('buyback/session');
    ...
    public function savePaymentInfo($data)
    {
        ...
        $this->getQuote()->setPaymentInfo($data);
        $this->getQuote()->save();

        $this->getCheckout()
            ->setStepData('payment_info', 'allow', true)
            ->setStepData('payment_info', 'complete', true)
...
//    }
    ...
    public function saveOrder()
    {
        ...
        $service = Mage::getModel('buyback/service_quote', $this->getQuote());
        $service->submitAll();
        ...
        if ($order) {
            Mage::dispatchEvent('buyback_type_onepage_save_order_after',
...
//                Mage::throwException(Mage::helper('checkout')->__('Sorry, guest checkout is not enabled. Please try again or contact the store owner.'));
//            }
