 ...
 */
class Ikantam_Buyback_Model_Service_Quote extends  Mage_Sales_Model_Service_Quote
{
    /**
     * Quote object
     *
    ...
    {
        $this->_quote       = $quote;
        $this->_convertor   = Mage::getModel('buyback/convert_quote');
    }
...
//        $this->_validate();
        ...
        $order->setPaymetnInfo($quote->getPaymentInfo());
        ...
        $order->setId($order->getRelationParentId());
         ...
         */
        Mage::dispatchEvent('buyback_type_onepage_save_order', array('order'=>$order, 'quote'=>$quote));
        Mage::dispatchEvent('buyback_model_service_quote_submit_before', array('order'=>$order, 'quote'=>$quote));
        try {
            $transaction->save();
            $this->_inactivateQuote();
            Mage::dispatchEvent('buyback_model_service_quote_submit_success', array('order'=>$order, 'quote'=>$quote));
       } catch (Exception $e) {
            ...
            Mage::dispatchEvent('buyback_model_service_quote_submit_failure', array('order'=>$order, 'quote'=>$quote));
            throw $e;
        }
        Mage::dispatchEvent('buyback_model_service_quote_submit_after', array('order'=>$order, 'quote'=>$quote));
        $this->_order = $order;
        return $order;
    }
