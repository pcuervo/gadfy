...
class  Ikantam_Buyback_Adminhtml_Buyback_Order_CreateController extends Mage_Adminhtml_Controller_Action
{
    /**
...
//        $this->setUsedModuleName('Mage_Sales');
     ...
     *
     * @return Ikantam_Buyback_Model_Adminhtml_Session_Quote
     */
     ...
     *
     * @return Ikantam_Buyback_Model_Adminhtml_Order_Create
     */
    ...
    protected function _initSession()
    {
        ...
        Mage::dispatchEvent('create_buyback_order_session_quote_initialized',
    ...
    protected function _processActionData($action = null)
    {
        ...
        Mage::dispatchEvent('adminhtml_buyback_order_create_process_data_before', $eventData);
        ...
        Mage::dispatchEvent('adminhtml_buyback_order_create_process_data', $eventData);
    ...
    {
        $this->_title($this->__('Buyback'))->_title($this->__('Orders'))->_title($this->__('New Buyback Order'));
        $this->_initSession();
        $this->loadLayout();

        $this->_setActiveMenu('buyback/order')
            ->renderLayout();
    }
    ...
    public function loadBlockAction()
    {
        ...
        if ($asJson) {
            $update->addHandle('adminhtml_buyback_order_create_load_block_json');
        } else {
            $update->addHandle('adminhtml_buyback_order_create_load_block_plain');
        }
        ...
        if ($block) {
            ...
            foreach ($blocks as $block) {
                $update->addHandle('adminhtml_buyback_order_create_load_block_' . $block);
            }
        }
    ...
    public function saveAction()
    {
        try {
            ...
            $paymentInfo = $this->getRequest()->getPost('payment_info');
            $this->_getQuote()->setPaymentInfo($paymentInfo);
            ...
            $this->_getSession()->clear();
            if ($this->_getSession()->getOrder()->getId()) { // if edit
                Mage::getSingleton('adminhtml/session')->addSuccess($this->__('The buyback order has been saved.'));
            } else {
                Mage::getSingleton('adminhtml/session')->addSuccess($this->__('The buyback order has been created.'));
            }

            if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/view')) {
                $this->_redirect('*/buyback_order/view', array('order_id' => $order->getId()));
            } else {
                $this->_redirect('*/buyback_order/index');
            }
            ...
            $this->_getSession()->addException($e, $this->__('Buyback Order saving error: %s', $e->getMessage()));
            $this->_redirect('*/*/');
        }
    }
